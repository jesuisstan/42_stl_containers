...
int main() {

	{
		...
		std::vector<int> vec_my_new(vec_my.begin(), vec_my.end() - 2);
		...
		std::vector<int>::iterator it = vec_my.begin();
		...
		std::vector<int> vec_to_copy_from_ft;
		...
		std::vector<int>::const_iterator cit = vec_my.begin();
		if (cit >= it)
			std::cout << "\nvector iterators are comparable\n";
		std::vector<int>::const_iterator cit2 = it;
		...
		printVec(test);
		std::vector<int>::iterator itt = test.begin() + 5;
		...
		std::vector<int>::reverse_iterator rit = vec_my.rbegin();
		std::cout << "rbegin() = " << *rit << std::endl;
		std::vector<int>::reverse_iterator rit2 = vec_my.rend();
		...
		std::cout << GREEN << "\ntotal time spent on std::vector testing = " << get_timestamp() - g_timestamp << " μs" << RESET << std::endl;;
...

	{
		...
		{	
			...
			std::map<int, std::string> map_orig;
			map_orig[0] = "qqq";
			map_orig[1] = "aaa";
			map_orig[2] = "bbb";
			map_orig[5] = "ccc";
			...
			std::map<int, std::string>::iterator it = map_orig.begin();
			it++;
			++it;
			std::cout << it->first << "\n";
		}
		...
		{	
			std::cout << USCORED << "\ntest insert pair:\n" << RESET;
			std::pair<int, std::string>			my_pair(8, "salut");
			std::map<int, std::string>			test;

			test.insert(my_pair);
			test.insert(std::pair<int, std::string>(-4, "cyber"));
			test.insert(std::pair<int, std::string>(2, "punk"));
			test.insert(std::pair<int, std::string>(3, "ain't"));
			test.insert(std::pair<int, std::string>(746, "dead"));
			test.insert(std::pair<int, std::string>(1, "yet"));
			std::cout << "\nmap containes: ";
			printMap(test);
			std::cout << "\nreversed map containes: ";
			printMapRev(test);
...

		{
			...
			mp.insert(my_pair);
			mp.insert(std::pair<int, std::string>(-4, "cyber"));
			mp.insert(std::pair<int, std::string>(2, "punk"));
			mp.insert(std::pair<int, std::string>(3, "ain't"));
			mp.insert(std::pair<int, std::string>(746, "dead"));
			mp.insert(std::pair<int, std::string>(1, "yet"));
			std::cout << "\nmap containes: ";
			printMap(mp);
...

		{
			...
			std::pair<int, std::string>			my_pair(-42, "salut");
			std::map<int, std::string>			mp;

			mp.insert(my_pair);
			mp.insert(std::pair<int, std::string>(-4, "cyber"));
			mp.insert(std::pair<int, std::string>(1, "punk"));
			mp.insert(std::pair<int, std::string>(3, "ain't"));
			mp.insert(std::pair<int, std::string>(21, "dead"));
			mp.insert(std::pair<int, std::string>(746, "yet"));
			std::cout << "\nmap containes: ";
			printMap(mp);
		...
		std::cout << GREEN << "\ntotal time spent on std::map testing = " << get_timestamp() - g_timestamp << " μs" << RESET << std::endl;;
...

	{
		...
		std::cout << GREEN << "\ntotal time spent on std::stack testing = " << get_timestamp() - g_timestamp << " μs" << RESET << std::endl;;
	...
	{
		std::set<int, std::greater<int> >	s0;
		...
		std::set<int, std::greater<int> >	s1(s0);
		...
		std::set<int, std::greater<int> >	s2 = s0;
	...
	std::cout << "\ntest access and iterators\n";
	{
		...
		std::set<int>::iterator	it = s0.begin();
		for (; it != s0.end(); ++it)
			std::cout << *it << " ";

		std::cout << "\niteration with reverse iterator \n";
		std::set<int>::reverse_iterator	it2 = ++s0.rbegin();
		for (; it2 != s0.rend(); ++it2)
			std::cout << *it2 << " ";
		std::cout << "\n";
	...
	std::cout << "\ntest equal_range()\n";
	{
		...
		std::cout << GREEN << "\ntotal time spent on std::set testing = " << get_timestamp() - g_timestamp << " μs" << RESET << std::endl;;
	}
